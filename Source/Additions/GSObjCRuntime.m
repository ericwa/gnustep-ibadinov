...
typedef struct __ParserStackElement
{
  const char *cursor;
  size_t      size;
...
typedef struct __WeirdInfoAccumulator
{
  size_t      size;
  unsigned    depth;
} WeirdInfoAccumulator;

static void
WeirdInfoAccumulatorAddInfo (WeirdInfoAccumulator *this, GSObjCTypeInfo info)
{
                                  ...
                                  GSObjCTypeInfo info)
{
  if (this->count == this->allocated)
    {
      void *newInfo = malloc(sizeof(GSObjCTypeInfo) * (this->allocated *= 2));
      memcpy(newInfo, this->info, sizeof(GSObjCTypeInfo) * this->count);
      free(this->info);
      this->info = newInfo;
    }
